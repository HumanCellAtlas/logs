...
apps/firehose_to_es_processor/add_log_group.sh $CLOUDTRAIL_LOG_GROUP_NAME $firehose_delivery_stream_arn $cwl_to_kinesis_role_arn

for group in `aws logs describe-log-groups | jq -r '.logGroups[] | .logGroupName'` ; do
  echo "Subscribing to: $group"
  apps/firehose_to_es_processor/add_log_group.sh $group $firehose_delivery_stream_arn $cwl_to_kinesis_role_arn || echo "Subscription failed: $group"
done
